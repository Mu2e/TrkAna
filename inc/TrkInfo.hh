...
namespace mu2e
{
  ...
  struct TrkInfoMC {
    bool valid; // whether this information is valid or not
    ...
    void reset() { valid = false;
      ndigi = ndigigood = nactive = nhits = nambig = pdg = gen  = proc= -1;  otime=0.0;
      prel = MCRelationship();
      opos = XYZVectorF();
      omom = XYZVectorF();
    }
  ...
  struct TrkInfoMCStep {
    bool valid; // true/false whether the data is valid
    ...
    TrkInfoMCStep() { reset(); }
    void reset() { valid = false; time = -1; mom = XYZVectorF(); pos = XYZVectorF(); }
  };

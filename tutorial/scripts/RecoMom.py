...
ent_mask = (branches['demfit.sid']==0)
time_cut = (branches['demlh.t0']>=700)
...
ax.hist(ak.flatten(branches['demfit.mom'][(ent_mask)]), bins=n_mom_bins, range=(min_mom,max_mom), label='all tracks', histtype='step')
ax.hist(ak.flatten(branches['demfit.mom'][(ent_mask) & (time_cut)]), bins=n_mom_bins, range=(min_mom,max_mom), label='t0>=700', histtype='step')

...
//
// > root -l TrkAna/examples/ROOTCommandLine.C
//
/////////////////////////
...
//
// > trkana->Draw("deent.mom>>hist(100,90,110)")
//
...
//
// > trkana->Draw("deent.mom>>hist(100,90,110)", "de.t0>700", "HIST SAME")
//
// Finally, one can Scan through the tree event by event:
//
// > trkana->Scan("deent.mom:de.t0", "de.t0>700")
//
{
  ...
  gROOT->ProcessLine("trkana->Draw(\"deent.mom>>hist(100,90,110)\")");
  gROOT->ProcessLine("hist->SetLineColor(kRed)");
  gROOT->ProcessLine("trkana->Draw(\"deent.mom>>hist2(100,90,110)\", \"de.t0>700\", \"HIST SAME\")");
  gROOT->ProcessLine("trkana->Scan(\"deent.mom:de.t0\", \"de.t0>700\")");
}
